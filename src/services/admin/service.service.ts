...
async function createService({
  ...
  const serviceTitle = serviceLevelData.title;

  await logService.createLog({
    organization: reqAuth.membership.organization._id,
    payload_snapshot: logService.encryptPayloadSnapshot({
      org,
      serviceLevelData: JSON.stringify(serviceLevelData)
    }),
    type: ServiceLogType.CREATE,
    activity: encryptFieldData(
      `${userFullName} (${userOrgName} ${reqAuth.role}) created service ${serviceTitle}`
    )
  });
...
async function updateService({
  ...
  const serviceTitle = existingService.title;
  ...
  await logService.createLog({
    ...
    activity: encryptFieldData(
      `${userFullName} (${userOrgName} ${reqAuth.role}) updated service ${serviceTitle}`
    )
  });
...
async function getServiceMetrics({
  ...
  const learnersData = await User.find({
    ...
    'organizations.approved_at': { $gte: period.previous.start }
...
async function updatePublishFieldService({
  ...
  const logServiceType = isPublished
  ...
  await logService.createLog({
    ...
    activity: encryptFieldData(
      generateLogDetails({
        ...
        type: logServiceType.toLowerCase()
      })
    )
  });
}
